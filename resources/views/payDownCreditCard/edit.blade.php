        ...
        $(document).ready(function() {
            ...
            $('#pay-down-credit-card-form').on('submit', function(e) {
                ...
                $.ajax({
                        ...
                        if (response.success) {
                            if (typeof show_toastr === 'function') {
                                show_toastr('success', response.message ||
                                    '{{ __('Credit card payment updated successfully') }}',
                                    'success');
                            }
                            setTimeout(() => window.location.href =
                                '{{ route('expense.index') }}', 500);
                        } else {
                            if (typeof show_toastr === 'function') {
                                show_toastr('error', response.message ||
                                    '{{ __('Error updating payment') }}', 'error');
                            }
                    ...
                    error: function(xhr) {
                        let message = '{{ __('Error updating payment') }}';
    ...
    <div class="modal fade" id="expense-modal" tabindex="-1" aria-labelledby="expenseModalLabel" aria-hidden="true"
        ...
        ">
                        <div class="TrowserHeader d-flex align-items-center">
                            ...
                            <h5 class="mb-0" style="font-size: 1.2rem;">{{ __('Pay down credit card') }}
                                #{{ $payment->id }}</h5>
                        </div>
                        <div class="TrowserHeader d-flex align-items-center">
                    ...
                    <form action="{{ route('paydowncreditcard.update', Crypt::encrypt($payment->id)) }}" method="POST"
                        id="pay-down-credit-card-form" enctype="multipart/form-data" class="w-100"
                        style="padding: 30px 30px; background: #ffffff;">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="card">
                                <div class="card-body" style="background:#f4f5f8;">
                                    ...
                                    <div class="row align-items-start">
                                        <!-- LEFT: Form Fields -->
                                        <div class="col-10">
                                            <p class="mb-3" style="font-size: 14px; color: #393a3d;">
                                                {{ __('Record payments made to your balance') }}</p>
                                            ...
                                            <div class="row mb-3">
                                                <div class="col-3">
                                                    <label class="form-label"
                                                        style="font-size: 12px; color: #6b7280;">{{ __('How much did you pay?') }}</label>
                                                    <input type="text" name="amount" id="pay-down-amount"
                                                        class="form-control" value="{{ $payment->amount }}"
                                                        placeholder="{{ __('Enter the amount') }}" required>
                                                </div>
                                                <div class="col-3">
                                                    <label class="form-label"
                                                        style="font-size: 12px; color: #6b7280;">{{ __('Date of payment') }}</label>
                                            ...
                                            <div class="row mb-3">
                                                <div class="col-6">
                                                    <label class="form-label"
                                                        style="font-size: 12px; color: #6b7280;">{{ __('What did you use to make this payment?') }}</label>
                                                    <select name="bank_account_id" id="bank_account_id"
                                                        class="form-control select" required>
                                                        <option value="">{{ __('Select bank account') }}</option>
                                                        @foreach ($bankAccounts as $id => $name)
                                                            <option value="{{ $id }}"
                                                                {{ $payment->bank_account_id == $id ? 'selected' : '' }}>
                                                                {{ $name }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            ...
                                            <div class="mt-4">
                                                <button type="button"
                                                    class="qbo-accordion-header expanded d-flex align-items-center gap-2 bg-transparent border-0 p-0"
                                                    style="font-size: 14px; font-weight: 600; color: #393a3d; cursor: pointer;">
                                                ...
                                                <div class="qbo-accordion-content show"
                                                    ...
                                                    <div style="font-size: 12px; color: #6b7280; margin-bottom: 8px;">
                                                        {{ __('Memo') }}</div>
                                                    <textarea name="memo" id="memo" class="form-control" style="max-width: 280px; min-height: 120px;">{{ $payment->memo }}</textarea>

                                                    <div class="mt-4">
                                                        <label
                                                            style="font-size: 12px; color: #6b7280; margin-bottom: 8px; display: block;">{{ __('Attachments') }}</label>
                                                        ...
                                                        <div class="attachment-zone" id="attachment-zone">
                                                            <a href="#" class="attachment-link"
                                                                id="attachment-add-link">
                                                                {{ __('Add attachment') }}
                                                            </a>
                                                            <div class="attachment-limit">{{ __('Max file size: 20 MB') }}
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="mt-2">
                                                            <a href="#"
                                                                style="color: #0077c5; font-size: 14px; text-decoration: none;">{{ __('Show existing') }}</a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="text-center mt-4">
                                                <a href="https://www.intuit.com/privacy/" target="_blank"
                                                    style="color: #0077c5; font-size: 14px; text-decoration: none;">{{ __('Privacy') }}</a>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-2 text-end">
                                            <div style="font-size: 12px; color: #6b7280; margin-bottom: 2px;">
                                                {{ __('Total paid') }}</div>
                                            <div id="totalPaidDisplay"
                                                style="font-size: 28px; font-weight: 700; color: #393a3d;">
        ...
        ">
                ...
                ">Cancel</a> --}}
                            {{-- <form action="{{ route('paydowncreditcard.destroy', Crypt::encrypt($payment->id)) }}"
                                ...
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn-danger-outline">{{ __('Delete') }}</button>
                            </form> --}}
                        </div>
                        ...
                        <div class="footer-right d-flex align-items-center gap-2">
                            <button type="submit" form="pay-down-credit-card-form"
                                class="btn btn-light btn-sm-qbo btn-qbo-save">{{ __('Save') }}</button>
                            <div class="btn-group">
                                <button type="submit" form="pay-down-credit-card-form"
                                    class="btn btn-success btn-sm-qbo"
                                    style="background-color: #2ca01c; border-color: #2ca01c;">{{ __('Save and close') }}</button>
                                <button type="button"

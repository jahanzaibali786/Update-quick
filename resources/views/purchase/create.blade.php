...
<script>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#add-item-line').on('click', function() {
                const newRow = `
                    <tr class="item-row">
                        ...
                        <td><input type="number" name="items[${itemLineCount}][amount]" class="form-control item-amount" step="0.01" value="0.00" readonly>
                            <input type="hidden" name="items[${itemLineCount}][discount]" class="form-control discount" value="0">
                        </td>
            ...
            // Calculate Total - sums category amounts and item amounts
            ...
            $(document).on('keyup change', '.item-qty, .item-rate, .quantity, .price', function() {
                var row = $(this).closest('tr');
                var qty = parseFloat(row.find('.item-qty, .quantity').val()) || 0;
                var rate = parseFloat(row.find('.item-rate, .price').val()) || 0;
                var amount = qty * rate;
                row.find('.item-amount, .amount').val(amount.toFixed(2));
                calculateTotal();
            });
            ...
            $('#bill-form').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            ...
                            if (typeof show_toastr === 'function') {
                                show_toastr('success',
                                    '{{ __('Purchase created successfully') }}', 'success');
                            }
                    ...
                    error: function(xhr) {
                        let message = '{{ __('Error creating purchase') }}';
                ...
                <div class="row">
                    <div class="d-flex justify-content-between align-items-center border-bottom"
                        ...
                        <div class="TrowserHeader d-flex align-items-center">
                            ...
                            <h5 class="mb-0" style="font-size: 1.2rem;">Purchase Order</h5>
                        </div>
                        <div class="TrowserHeader d-flex align-items-center">
                    ...
                    {{ Form::open(['route' => 'purchase.store', 'enctype' => 'multipart/form-data', 'class' => 'w-100', 'style' => 'padding: 30px 30px; background: #ffffff;', 'id' => 'bill-form']) }}
                    <div class="col-12">
                                ...
                                <div class="row align-items-start">
                                            ...
                                            <div class="col-md-4">
                                                <div class="form-group mb-3">
                                                    ...
                                                    <select name="vendor_id" class="form-control select"
                                                        id="vendor_selector" required data-create-url="{{ route('vender.create') }}"
                                                        ...
                                                        @foreach ($vendors as $id => $vendor)
                                                            <option value="{{ $id }}"
                                                                {{ isset($vendorId) && $vendorId == $id ? 'selected' : '' }}>
                                                                {{ $vendor }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                ...
                                                <div class="form-group mb-3">
                                                    ...
                                                    <textarea class="form-control" name="mailing_address" id="vendor_address" rows="5" readonly
                                            ...
                                            <div class="col-md-8">
                                                ...
                                                <div class="row">
                                                    <div class="col-md-4 mb-3">
                                                        <div class="d-flex justify-content-between">
                                                            ...
                                                            <a href="#"
                                                                class="text-primary small btn-link">Cc/Bcc</a>
                                                        </div>
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-4 mb-3">
                                                        ...
                                                        <label
                                                            class="form-label mt-3">{{ __('Shipping address') }}</label>
                                                        <textarea class="form-control" name="ship_to_address" id="shipping_address" rows="3"></textarea>
                                                    </div>

                                                    <!-- PO Date / Ship Via Column -->
                                                    <div class="col-md-3 mb-3">
                                                        <label class="form-label">{{ __('Purchase Order date') }}</label>
                                                        <input type="date" name="po_date" class="form-control"
                                                            required value="{{ date('Y-m-d') }}">

                                                        <label class="form-label mt-3">{{ __('Ship Via') }}</label>
                                    ...
                                    <div class="col-2 text-end">
                                        <div class="d-flex flex-column align-items-end">
                                            ...
                                            <p class="h3 mb-0 grand-total-display totalAmount"
                                                style="font-size:36px;font-weight:900;" id="grand-total-display">$0.00</p>
                                            <input type="hidden" name="total" class="totalAmount" value="0">
                                            <input type="hidden" name="subtotal" class="subTotal" value="0">
                                            <input type="hidden" name="tax_total" class="totalTax" value="0">
                                        </div>
                                    </div>
                                </div>

                                <!-- ========== BOTTOM FIELDS (Removed as they are now in top section) ========== -->
                                                    ...
                                                    <tbody data-repeater-list="categories" id="category-tbody">
                                                        @for ($i = 0; $i < 2; $i++)
                                                            <tr data-repeater-item>
                                                                ...
                                                                <td>
                                                                    {{ Form::number("category[{$i}][amount]", null, ['class' => 'form-control text-end category-amount accountAmount', 'step' => '0.01']) }}
                                                                </td>
                                                    ...
                                                    <tbody data-repeater-list="items" id="item-tbody">
                                                        @for ($i = 0; $i < 2; $i++)
                                                            <tr data-repeater-item>
                                                                ...
                                                                <td>
                                                                    {{ Form::textarea("items[{$i}][description]", null, [
                                                                        'class' => 'form-control item-description',
                                                                        'rows' => 1,
                                                                    ]) }}
                                                                </td>
                                                                <td>
                                                                    {{ Form::number("items[{$i}][quantity]", 1, [
                                                                        'class' => 'form-control text-center item-qty quantity',
                                                                        'min' => 1,
                                                                    ]) }}
                                                                </td>
                                                                <td>
                                                                    {{ Form::number("items[{$i}][price]", null, [
                                                                        'class' => 'form-control text-end item-rate price',
                                                                        'step' => '0.01',
                                                                        'placeholder' => '0.00',
                                                                    ]) }}
                                                                </td>
                                                                ...
                                                                <td>
                                                                    {{ Form::number("items[{$i}][amount]", null, ['class' => 'form-control text-end item-amount amount', 'step' => '0.01']) }}
                                                                    {{ Form::hidden("items[{$i}][discount]", 0, ['class' => 'form-control discount']) }}
                                                                    {{ Form::hidden("items[{$i}][itemTaxPrice]", 0, ['class' => 'itemTaxPrice']) }}
                                                                    {{ Form::hidden("items[{$i}][itemTaxRate]", 0, ['class' => 'itemTaxRate']) }}
                                                                </td>
                                                                {{-- <td class="text-center">
                                                                    {{ Form::checkbox("items[{$i}][billable]", 1, false, ['class' => 'form-check-input']) }}
                                                                </td>
                                                                <td class="text-center">
                                                                    {{ Form::checkbox("items[{$i}][tax]", 1, false, ['class' => 'form-check-input']) }}
                                                                </td> --}}
                        ...
                        <div class="row" style="padding:20px;">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="vendor_message" class="form-label">{{ __('Vendor Message') }}</label>
                                    {{ Form::textarea('vendor_message', null, [
                                        'class' => 'form-control',
                                        'rows' => '5',
                                        'placeholder' => '',
                                    ]) }}

...
<div class="qbo-po-container">
    {{ Form::model($purchase, ['route' => ['purchase.update', $purchase->id], 'method' => 'PUT', 'enctype' => 'multipart/form-data', 'id' => 'po-form']) }}
    ...
    <div class="qbo-po-header">
        ...
        <div class="qbo-balance-section">
            ...
            <div class="qbo-total-badge" id="grand-total-display">${{ number_format($purchase->getTotal(), 2) }}</div>
        </div>
    </div>
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <div class="mb-3">
                    ...
                    <select name="vendor_id" class="form-control qbo-input" id="vendor_selector" required>
                        ...
                        @foreach ($vendors as $id => $vendor)
                            <option value="{{ $id }}" {{ $purchase->vender_id == $id ? 'selected' : '' }}>
                                {{ $vendor }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="status" class="form-control qbo-input">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control qbo-input" name="mailing_address" id="vendor_address" rows="4">{{ $purchase->mailing_address }}</textarea>
                </div>
            </div>
            ...
            <div class="col-md-6">
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="email" name="vendor_email" class="form-control qbo-input"
                            value="{{ $purchase->vendor_email }}" placeholder="{{ __('Separate emails with a comma') }}">
                    </div>
                    <div class="col-md-6 mb-3">
                ...
                <div class="mb-3">
                    ...
                    <select name="ship_to" class="form-control qbo-input">
                        ...
                        @foreach ($customers ?? [] as $customer)
                            <option value="{{ $customer->id }}" {{ $purchase->ship_to == $customer->id ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control qbo-input" name="ship_to_address" rows="3">{{ $purchase->ship_to_address }}</textarea>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" name="ship_via" class="form-control qbo-input"
                            value="{{ $purchase->ship_via }}" placeholder="FedEx, UPS, etc.">
                    </div>
                    <div class="col-md-4 mb-3">
    ...
    <div class="qbo-form-section">
            ...
            <div class="qbo-grid-content">
                <table class="qbo-grid-table">
                    ...
                    <tbody id="category-tbody">
                        @foreach ($purchase->accounts as $index => $account)
                            <tr class="category-row">
                                ...
                                <td>
                                    <select name="category[{{ $index }}][account_id]"
                                        ...
                                        @foreach ($chartAccounts as $id => $chartAccount)
                                            <option value="{{ $id }}"
                                                {{ $account->chart_account_id == $id ? 'selected' : '' }}>
                                                {{ $chartAccount }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <td>
                                    <input type="text" name="category[{{ $index }}][description]"
                                        class="form-control form-control-sm" value="{{ $account->description }}">
                                </td>
                                <td>
                                    <input type="number" name="category[{{ $index }}][amount]"
                                    ...
                                    <select name="category[{{ $index }}][customer_id]"
                                        ...
                                        @foreach ($customers ?? [] as $customer)
                                            <option value="{{ $customer->id }}"
                                                {{ $account->customer_id == $customer->id ? 'selected' : '' }}>
                                                {{ $customer->name }}</option>
                                        @endforeach
                                    </select>
                                </td>
                            ...
                            <tr class="item-row">
                                ...
                                <td>
                                    <select name="items[{{ $index }}][product_id]"
                                        class="form-control form-control-sm item-product">
                                        <option value="">{{ __('Select') }}</option>
                                        @foreach ($product_services as $id => $product)
                                            <option value="{{ $id }}"
                                                {{ $item->product_id == $id ? 'selected' : '' }}>{{ $product }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <td>
                                    <input type="number" name="items[{{ $index }}][quantity]"
                                        class="form-control form-control-sm item-qty" step="1"
                                        value="{{ $item->quantity }}">
                                </td>
                                <td>
                                    <input type="number" name="items[{{ $index }}][price]"
                                        class="form-control form-control-sm item-rate" step="0.01"
                                        value="{{ $item->price }}">
                                </td>
                                <td>
                                    <input type="number" name="items[{{ $index }}][amount]"
                                        class="form-control form-control-sm item-amount" step="0.01"
        ...
        <div class="row mt-4">
            <div class="col-md-4">
                <div class="mb-3">
                    ...
                    <textarea name="vendor_message" class="form-control qbo-input" rows="3">{{ $purchase->vendor_message }}</textarea>
                </div>
            </div>
            <div class="col-md-4">
                <div class="mb-3">
                    <label class="qbo-label">{{ __('Memo') }}</label>
                    <textarea name="notes" class="form-control qbo-input" rows="3">{{ $purchase->notes }}</textarea>
                </div>
            </div>
            <div class="col-md-4">
                <div class="mb-3">
                ...
                <div class="qbo-total-section mt-3">
                    <div class="qbo-total-row grand-total">
                        <span>{{ __('Total') }}</span>
                        <span id="total-display">${{ number_format($purchase->getTotal(), 2) }}</span>
                    </div>
                </div>
            </div>
        </div>

        <input type="hidden" name="subtotal" id="subtotal" value="{{ $purchase->getSubTotal() }}">
        <input type="hidden" name="total" id="total" value="{{ $purchase->getTotal() }}">
        ...
        <div class="qbo-action-buttons">
            ...
            <div>
                <button type="submit" class="btn-qbo btn-qbo-save">{{ __('Update') }}</button>
                <button type="submit" class="btn-qbo btn-qbo-save-close" name="save_and_close"
                    value="1">{{ __('Update and close') }}</button>
            </div>
        </div>
    </div>
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.copy-row', function() {
            ...
            // Remove hidden ID field (don't duplicate existing record IDs)
        ...
        $('#clear-category-lines').on('click', function() {
            if (confirm('{{ __('Are you sure you want to clear all category lines?') }}')) {
        ...
        $('#clear-item-lines').on('click', function() {
            if (confirm('{{ __('Are you sure you want to clear all item lines?') }}')) {
                $('#item-tbody').empty();
                itemLineCount = 0;
                calculateTotal();
            }

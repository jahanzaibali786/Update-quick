...
@section('page-title')
    {{ __('Refund Receipt Edit') }}
@endsection

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{ __('Dashboard') }}</a></li>
    <li class="breadcrumb-item"><a href="{{ route('refund-receipt.index') }}">{{ __('Refund Receipt') }}</a></li>
    <li class="breadcrumb-item">{{ __('Refund Receipt Edit') }}</li>
@endsection
...
@push('script-page')
    ...
    @push('script-page')
                ...
                $('#invoice-form').on('submit', function(e) {
                    ...
                    $('#sortable-table').children('tbody').each(function() {
                        ...
                        if ($productRow.length) {
                            ...
                            // Check if this item came from an estimate (or other source later)
            ...
            $(document).on('change', '#customer_id', function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('#edit-cutomer-link').removeClass('d-none');
                        if (typeof data === 'object') {
                            ...
                            if (data.customer) {
                                ...
                                //shipping
                                var shippingAddress = '';
                                if (data.customer.shipping_name) {
                                    shippingAddress += data.customer.shipping_name + '\n';
                                }
                                if (data.customer.shipping_address) {
                                    shippingAddress += data.customer.shipping_address + '\n';
                                }
                                if (data.customer.shipping_city) {
                                    shippingAddress += data.customer.shipping_city;
                                    if (data.customer.shipping_state) {
                                        shippingAddress += ', ' + data.customer.shipping_state;
                                    }
                                    if (data.customer.shipping_zip) {
                                        shippingAddress += ' ' + data.customer.shipping_zip;
                                    }
                                    shippingAddress += '\n';
                                }
                                if (data.customer.shipping_country) {
                                    shippingAddress += data.customer.shipping_country;
                                }

                                $('textarea[name="ship_to"]').val(shippingAddress.trim());

                                // Show the ship-to section
                                $('#ship-to-section').show();
                            }
                        } else if (data != '') {
            ...
            if ($(selector + " .repeater").length) {
                ...
                var $repeater = $(selector + ' .repeater').repeater({
                    ...
                    show: function() {
                        ...
                        // 🔹 NEW: fill new product row from a suggestion (Estimate)
        ...
        <script>
            // when a product/service is selected
            $(document).on('change', '.item', function() {
                // If we are programmatically setting the value (e.g. from suggestion), skip AJAX
                ...
                // if user clears the select, wipe the row and recalc
                if (!iteams_id) {
                ...
                $.ajax({
                    ...
                    cache: false,
                    success: function(data) {
                        // support both JSON string and already-parsed object
                        if (typeof data === 'string') {
                            try {
                                data = JSON.parse(data);
                            } catch (e) {
                                console.error('Could not parse product response', e);
                                return;
                            }
                        }
                        ...
                        // optional tax badges / ids (kept so your backend still gets tax IDs)
                        ...
                        if (item.taxes && item.taxes.length) {
                            for (var i = 0; i < item.taxes.length; i++) {
                                taxesHtml += '<span class="badge bg-primary mt-1 mr-2">' +
            ...
            // qty or rate changed -> recompute that row's amount, then totals
            ...
            // user edits Amount directly -> just recompute totals
            $(document).on('keyup change', '.amount', function() {
                recalcTotals();
            });

            // tax checkbox toggled -> affects taxable subtotal
            ...
            // when a repeater row is removed via data-repeater-delete
            $(document).on('click', '[data-repeater-delete]', function() {
                // actual DOM removal happens in repeater.hide()
                // just wait a tick then renumber + recalc
                setTimeout(function() {
                    renumberInvoiceLines();
                    recalcTotals();
                }, 200);
        ...
        <script>
            $(function() {
                ...
                // ---------- helpers to build special bodies ----------

                function createSubtotalBody(initialAmount) {
                    var amountText = (typeof initialAmount === 'string') ?
                        initialAmount :
                        '0.00';
                ...
                // allow optional initial text
                function createTextBody(initialText) {
                    ...
                    // if text passed from proposal, set it
                    if (initialText) {
                        $tbody.find('input[type="text"]').val(initialText);
                    }

                    return $tbody;
                }

                // expose for other scripts (suggestion panel)
                window.qbCreateSubtotalBody = createSubtotalBody;
                window.qbCreateTextBody = createTextBody;
...
@section('content')
    <div class="modal fade" style="background-color: #ffffff;" id="invoice-modal" tabindex="-1"
        aria-labelledby="invoiceModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-fullscreen">
            <div class="modal-content">
                <div class="invoice-container">
                    {{ Form::model($refundReceipt, ['route' => ['refund-receipt.update', $refundReceipt->id], 'method' => 'PUT', 'id' => 'invoice-form', 'files' => true]) }}
                    <input type="hidden" name="_token" id="token" value="{{ csrf_token() }}">
                    <input type="hidden" name="id" value="{{ $refundReceipt->id }}">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12" id="invoice-main-col">
                            <div class="invoice-card">
                                {{-- Refund Receipt Header Section (QBO Style) --}}
                                <div class="refund-receipt-header" style="padding: 24px 32px; background: #ebf4fa;">
                                    ...
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            {{ Form::text('customer_email_display', $refundReceipt->customer_email ?? ($refundReceipt->customer->email ?? ''), [
                                                'class' => 'form-control',
                                                'id' => 'customer_email_field',
                                                'placeholder' => 'Email (Separate emails with a comma)',
                                            ]) }}
                                            {{ Form::hidden('customer_email', $refundReceipt->customer_email ?? ($refundReceipt->customer->email ?? ''), ['id' => 'customer_email']) }}
                                        </div>
                                        <div class="col-md-5 text-end">
                                            <div style="margin-top: 20px;">
                                                <label style="font-size: 12px; color: #6b6c72; text-transform: uppercase; letter-spacing: 0.5px;">{{ __('AMOUNT') }}</label>
                                                <div style="font-size: 28px; font-weight: 500; color: #393a3d;" id="header-amount-display">${{ number_format($refundReceipt->total_amount ?? 0, 2) }}</div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-md-3">
                                            ...
                                            {{ Form::textarea('billing_address', $refundReceipt->billing_address ?? $billingAddress ?? '', [
                                                'class' => 'form-control',
                                                'id' => 'billing_address',
                                                'rows' => 3,
                                                'style' => 'resize: none;',
                                        ...
                                        <div class="col-md-3">
                                            ...
                                            {{ Form::date('issue_date', $refundReceipt->issue_date ?? date('Y-m-d'), [
                                                'class' => 'form-control',
                                                'id' => 'issue_date',
                                                'required' => 'required',
                                                'style' => 'width: 185px;',
                                        ...
                                        <div class="col-md-3 offset-md-3 text-end">
                                            ...
                                            {{ Form::text('location_of_sale', $refundReceipt->location_of_sale ?? (Auth::user()->address ?? '123 Sierra Way, San Pablo CA 87999'), [
                                                'class' => 'form-control',
                                                'id' => 'location_of_sale',
                                            ]) }}
                                        </div>
                                    ...
                                    <div class="row mb-2">
                                        <div class="col-md-3">
                                            ...
                                            {{ Form::select('payment_method', $paymentMethods ?? ['' => 'Choose payment method', 'Cash' => 'Cash', 'Check' => 'Check', 'Credit Card' => 'Credit Card', 'Debit Card' => 'Debit Card', 'Bank Transfer' => 'Bank Transfer', 'Other' => 'Other'], $refundReceipt->payment_method ?? null, [
                                                'class' => 'form-select',
                                                'id' => 'payment_method',
                                        ...
                                        <div class="col-md-3">
                                            ...
                                            {{ Form::select('refund_from', $bankAccounts ?? ['' => 'Choose an account'], $refundReceipt->refund_from ?? null, [
                                                'class' => 'form-select',
                                                'id' => 'refund_from',
                                    ...
                                    {{ Form::hidden('category_id', $refundReceipt->category_id ?? 1) }}
                                    {{ Form::hidden('ref_number', $refundReceipt->ref_number ?? '') }}
                                    {{ Form::hidden('recurring', $refundReceipt->recurring ?? 'no', ['id' => 'recurring']) }}
                                </div>
                                    ...
                                    <div class="bottom-section row">
                                        <div class="left-section col-md-4">
                                            ...
                                            <div class="info-field">
                                                <label>{{ __('Message displayed on refund receipt') }}</label>
                                                {{ Form::textarea('note', $refundReceipt->memo ?? '', [
                                                    'class' => 'form-control',
                                                    'rows' => 3,
                                            ...
                                            <div class="info-field">
                                                <label>{{ __('Message displayed on statement') }}</label>
                                                {{ Form::textarea('memo', $refundReceipt->statement_memo ?? '', [
                                                    'class' => 'form-control',
                                                    'rows' => 3,
                                                ]) }}
                                            </div>
                                        ...
                                        <div class="totals-section col-md-4">
                                            ...
                                            <div class="total-row final" style="border-top: 1px solid #e4e4e7; padding-top: 16px; margin-top: 16px;">
                                                ...
                                                <span class="totalAmount" style="font-weight: 600;">${{ number_format($refundReceipt->total_amount ?? 0, 2) }}</span>
                                            </div>

                                            {{-- Total Amount Refunded --}}
                                            <div class="total-row" style="padding-top: 8px;">
                                                <span>{{ __('Total Amount Refunded') }}</span>
                                                <span class="totalAmountRefunded">${{ number_format($refundReceipt->total_amount_refunded ?? $refundReceipt->total_amount ?? 0, 2) }}</span>
                                            </div>
                                        </div>
...
"><svg
                                    ...
                                    <button type="button" class="suggestions-close"><svg
                                            xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            color="currentColor" width="24px" height="24px" focusable="false"
                                            aria-hidden="true" class="">
                                            <path fill="currentColor"
                                    ...
                                    <div class="suggestions-filter-panel" id="suggestions-filter-panel">
                                        <div class="suggestions-filter-header">
                                            <div class="suggestions-filter-actions">
                                                <button type="button" class="suggestions-filter-reset"
                                                    id="suggestions-filter-reset">
                                                    Reset
                                                </button>
                                                <button type="button" class="suggestions-filter-apply"
                                                    id="suggestions-filter-apply">
                                                    Apply filter
                                                </button>
                                            </div>
                            ...
                            <style>
                                /* main content scrolls separately from suggestions */
                                #invoice-main-col {
        ...
        $(function() {
            ...
            function insertProposalItem(item, $insertBefore, estimateId) {
                ...
                if (desc.toLowerCase() === 'subtotal') {
                    if (typeof window.qbCreateSubtotalBody === 'function') {
                        // amount from DB is not critical; table will recalc
                        var $body = window.qbCreateSubtotalBody('0.00');
                ...
                // TEXT row
                if (desc.length && typeof window.qbCreateTextBody === 'function') {
                    var $bodyText = window.qbCreateTextBody(desc);
            ...
            // ========================================
            // Populate Email and Billing Address on Customer Selection
            // ========================================
            ...
            // ========================================
            // Populate Line Items from Refund Receipt Data (Edit Mode)
            // ========================================
            @if(isset($refundReceiptData) && !empty($refundReceiptData['items']))
            var refundReceiptData = @json($refundReceiptData);
            
            function populateLineItems() {
                var items = refundReceiptData.items || [];
                ...
                var $repeater = $('[data-repeater-list]');
                if (!$repeater.length) {
                    console.log('Repeater not found');
                    return;
                }
                
                // Remove empty rows first (keep only the template row if needed)
                var $existingRows = $repeater.find('[data-repeater-item]');
                ...
                items.forEach(function(item, index) {
                    ...
                    if (index === 0 && $existingRows.length > 0) {
                        ...
                        // Clone from first row or create new
                        if ($existingRows.length > 0) {
                            $row = $existingRows.first().clone();
                            $row.insertAfter($repeater.find('[data-repeater-item]').last());
                        } else {
                            // Click add button to create new row
                            $('[data-repeater-create]').click();
                            $row = $repeater.find('[data-repeater-item]').last();
                        }
                    }
                    
                    if ($row && $row.length) {
                        // Populate product/service dropdown
                        var $productSelect = $row.find('select[name*="item"], select.item, .item-select');
                        if ($productSelect.length) {
                            $productSelect.val(item.item);
                        }
                        
                        // Populate description
                        var $description = $row.find('input[name*="description"], textarea[name*="description"], .description');
                        if ($description.length) {
                            $description.val(item.description || '');
                        }
                        
                        // Populate quantity
                        var $quantity = $row.find('input[name*="quantity"], .quantity');
                        if ($quantity.length) {
                            $quantity.val(item.quantity || 1);
                        }
                        
                        // Populate price
                        var $price = $row.find('input[name*="price"], .price');
                        if ($price.length) {
                            $price.val(item.price || 0);
                        }
                        
                        // Populate amount
                        var $amount = $row.find('input[name*="amount"], .amount');
                        if ($amount.length) {
                            $amount.val(item.amount || (item.quantity * item.price) || 0);
                        }
                        
                        // Populate tax select
                        var $tax = $row.find('select[name*="tax"], .tax-select');
                        if ($tax.length && item.tax) {
                            $tax.val(item.tax);
                        }
                        
                        // Populate taxable checkbox
                        var $taxable = $row.find('input[type="checkbox"][name*="taxable"], input[type="checkbox"].taxable, .tax-checkbox');
                        if ($taxable.length) {
                            // Check the checkbox if taxable is 1 or '1' or true
                ...
                setTimeout(function() {
                    ...
                    if (typeof updateHeaderAmount === 'function') {
                        updateHeaderAmount();
                    }
                }, 200);
            }
            
            // Wait for repeater to be ready, then populate
            setTimeout(populateLineItems, 800);
            @endif
        });
    </script>
@endsection

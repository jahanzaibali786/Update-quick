{{-- resources/views/terms/edit-right.blade.php --}}
{{ Form::model($paymentTerm, ['route' => ['payment-terms.update', $paymentTerm->id], 'method' => 'PUT', 'id' => 'termForm', 'class' => 'term-drawer-wrapper h-100']) }}

    {{-- HEADER --}}
    <div class="term-drawer-header d-flex align-items-center justify-content-between border-bottom px-4 py-3">
        <h5 class="mb-0">{{ __('Edit Term') }}</h5>
        ...
        <div class="term-drawer-content p-4">
            ...
            <div class="mb-4">
                <div class="form-check mb-3">
                    <input class="form-check-input" type="radio" name="type" id="type_fixed_days" value="fixed_days" {{ $paymentTerm->type === 'fixed_days' ? 'checked' : '' }}>
                    <label class="form-check-label" for="type_fixed_days">
                        {{ __('Due in fixed number of days') }}
                    </label>
                </div>
                <div class="term-field-container ms-4 mb-3" id="fixed_days_field" style="{{ $paymentTerm->type !== 'fixed_days' ? 'display: none;' : '' }}">
                    <div class="row align-items-center">
                        <div class="col-auto">
                            {{ Form::number('due_in_days', $paymentTerm->due_in_days ?? 30, ['class' => 'form-control', 'style' => 'width: 80px;', 'min' => 0]) }}
                        </div>
                        <div class="col-auto">
                ...
                <div class="form-check mb-3">
                    <input class="form-check-input" type="radio" name="type" id="type_day_of_month" value="day_of_month" {{ in_array($paymentTerm->type, ['day_of_month', 'next_month_if_within']) ? 'checked' : '' }}>
                    <label class="form-check-label" for="type_day_of_month">
                        {{ __('Due by certain day of the month') }}
                    </label>
                </div>
                <div class="term-field-container ms-4 mb-3" id="day_of_month_field" style="{{ !in_array($paymentTerm->type, ['day_of_month', 'next_month_if_within']) ? 'display: none;' : '' }}">
                    <div class="row align-items-center mb-3">
                        <div class="col-auto">
                            {{ Form::number('day_of_month', $paymentTerm->day_of_month ?? 15, ['class' => 'form-control', 'style' => 'width: 80px;', 'min' => 1, 'max' => 31]) }}
                        </div>
                        <div class="col-auto">
                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-auto">
                            {{ Form::number('cutoff_days', $paymentTerm->cutoff_days, ['class' => 'form-control', 'style' => 'width: 80px;', 'min' => 0]) }}
                        </div>
                        <div class="col-auto">
...
<script>
    (function ($) {
        ...
        $('input[name="type"]').on('change', function() {
            ...
            $('#fixed_days_field, #day_of_month_field, #next_month_field').hide();
            ...
            } else if (selectedType === 'next_month_if_within') {
                $('#day_of_month_field').show();
                $('#next_month_field').show();
            }
        });

...
<script>
    $(document).ready(function() {
        ...
        // Handle payee selection change to populate mailing address
        $('#payee_all').on('change', function() {
            var payeeValue = $(this).val();
            
            if (!payeeValue) {
                $('textarea[name="mailing_address"]').val('');
                return;
            }

            $.ajax({
                url: '{{ route("expense.payee.address") }}',
                type: 'POST',
                headers: {
                    'X-CSRF-TOKEN': jQuery('#token').val()
                },
                ...
                success: function(response) {
                    if (response.address) {
                        $('textarea[name="mailing_address"]').val(response.address);
                    } else {
                        $('textarea[name="mailing_address"]').val('');
                    }
                },
                error: function(xhr) {
                    console.error('Error fetching payee address:', xhr);
                }
            });
        });
    });
                ...
                <div class="row" style="margin-top: 0px;">
                    <div class="col-2">
                        <div class="form-group mb-0">
                            {{ Form::label('mailing_address', __('Mailing address'), ['class' => 'form-label']) }}
                            {{ Form::textarea('mailing_address', null, ['class' => 'form-control', 'rows' => 4, 'style' => 'resize: none;']) }}
                        </div>
                    </div>
                    <div class="col-2">
                        <div class="form-group mb-0">
                    ...
                    <div class="col-2">
                        <div class="form-group mb-0">
                            <label for="ref_no" class="form-label">{{ __('Check no.') }}</label>
                            <input type="text" id="ref_no" name="ref_no" class="form-control">
                        </div>
                        <div class="form-check mt-2">
                            <input class="form-check-input" type="checkbox" value="1" id="print_later" name="print_later">
                            <label class="form-check-label" for="print_later">
                                {{ __('Print later') }}
                            </label>
                        </div>
                    </div>
...
<button type="button" class="btn btn-link text-success p-2 btn-cancel-custom" data-bs-dismiss="modal" style="
                    background: #fff;

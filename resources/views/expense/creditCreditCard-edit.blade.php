...
@php $vendorId = $bill->vender_id; @endphp
@section('page-title')
    {{__('Edit Credit Credit Card')}}
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{route('dashboard')}}">{{__('Dashboard')}}</a></li>
    <li class="breadcrumb-item"><a href="{{route('creditcreditcard.index')}}">{{__('Credit Credit Card')}}</a></li>
    <li class="breadcrumb-item">{{__('Edit Credit Credit Card')}}</li>
@endsection
    ...
    $(document).ready(function() {
        ...
        $('#bill-form').on('submit', function(e) {
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    let message = '{{ __('Error creating expense') }}';
...
<div class="row">
    <div class="d-flex justify-content-between align-items-center border-bottom"
        ...
        <div class="TrowserHeader d-flex align-items-center">
            ...
            <h5 class="mb-0" style="font-size: 1.2rem;">Credit Credit Card</h5>
        </div>
        <div class="TrowserHeader d-flex align-items-center">
    ...
    {{ Form::model($bill, ['route' => ['creditcreditcard.update', $bill->id], 'method' => 'PUT', 'class' => 'w-100', 'style' => 'padding: 30px 30px; background: #ffffff;', 'id' => 'bill-form']) }}
    <div class="col-12">
        ...
        <div class="card">
            <div class="card-body" style="background:#f4f5f8;">
                ...
                <style>
                    ...
                    .bill-address-field textarea {
                        resize: none;
                        height: 90px;
                    }
                    .bill-terms-field, .bill-date-field, .bill-billno-field {
                        width: 120px;
                    }
                    .bill-toggle-row {
                        display: flex;
                    ...
                    .bill-collapsible.collapsed {
                        max-height: 0;
                    }
                </style>

                <div class="bill-header-section">
                    {{-- Top Row: Payee + Bank/Credit Account + Amount (QBO Credit Card Credit style) --}}
                    <div class="bill-header-top" style="display: flex; align-items: flex-start; gap: 24px;">
                        ...
                        <div class="bill-field-group" style="flex: 0 0 220px;">
                            <label>{{ __('Bank/Credit account') }}</label>
                            <select name="account_id" id="account_id" class="form-control select2" required>
                                <option value="">{{ __('Select account') }}</option>
                                @foreach ($accounts as $id => $name)
                                    @if($id && $id != '__add__' && $id != '')
                                        <option value="{{ $id }}" {{ isset($bankAccount) && $bankAccount->id == $id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
                        <div class="bill-balance-col" style="text-align: right; margin-left: auto;">
                            <div class="bill-balance-label" style="font-size: 11px; color: #6b6c72; text-transform: uppercase; font-weight: 600;">{{ __('AMOUNT') }}</div>
                            <div class="bill-balance-amount grand-total-display" style="font-size: 28px; font-weight: 600; color: #393a3d;">{{ \Auth::user()->priceFormat($bill->total) }}</div>
                        </div>
                    </div>

                    {{-- Second Row: Payment Date + Ref no. (QBO Credit Card Credit style) --}}
                    <div class="bill-collapsible" id="billDetailsCollapsible" style="margin-top: 16px;">
                        <div class="bill-details-section" style="display: flex; gap: 16px; align-items: flex-end;">
                            <div class="bill-field-group" style="flex: 0 0 150px;">
                                <label>{{ __('Payment Date') }}</label>
                                <input type="date" name="bill_date" id="bill_date" class="form-control" required value="{{ $bill->bill_date }}">
                            </div>
                            <div class="bill-field-group" style="flex: 0 0 150px;">
                                <label>{{ __('Ref no.') }}</label>
                                <input type="text" name="order_number" class="form-control" placeholder="" value="{{ $bill->order_number }}">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bill-toggle-row">
                        <button type="button" class="bill-toggle-btn" id="billToggleBtn" onclick="toggleBillDetails()">
<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" color="currentColor" width="20px" height="20px" focusable="false" aria-hidden="true" class=""><path fill="currentColor" d="M18.014 15.991a1 1 0 0 1-.708-.294l-5.285-5.3-5.3 5.285a.999.999 0 1 1-1.412-1.416l6.008-5.99a1 1 0 0 1 1.414 0l5.992 6.008a1 1 0 0 1-.708 1.706z"></path></svg>
                        </button>
                    </div>
                </div>
                ...
                <script>
                    function toggleBillDetails() {
                        var section = document.getElementById('billDetailsCollapsible');
                        var btn = document.getElementById('billToggleBtn');
                    ...
                    $(document).ready(function() {
                        ...
                        var vendorData = @json($vendorOptions);
                        
                        // Handle Vendor Change (for Credit Credit Card, mainly for display consistency)
                        $('#vender').on('change', function() {
                            var vendorId = $(this).val();
                            // Clear any previous selection data
                            // For Credit Credit Card we don't auto-populate address/terms
                        });

                        // Handle Terms Change -> Calculate Due Date
                        $('#terms, #bill_date').on('change', function() {
                            var termId = $('#terms').val();
                            var billDateVal = $('#bill_date').val();
                            
                            if (termId && billDateVal && termsData) {
                                var term = termsData.find(t => t.id == termId);
                                if (term) {
                                    var billDate = new Date(billDateVal);
                                    var dueDate = new Date(billDate);
                                    
                                    if (term.type === 'fixed_days') {
                                        dueDate.setDate(billDate.getDate() + parseInt(term.due_in_days));
                                    } else if (term.type === 'day_of_month') {
                                        var day = parseInt(term.day_of_month);
                                        dueDate.setDate(day);
                                        if (dueDate < billDate) {
                                            dueDate.setMonth(dueDate.getMonth() + 1);
                                        }
                                    } else if (term.type === 'next_month_if_within') {
                            ...
                            <div class="table-responsive">
                                <table class="table align-middle" id="category-table">
                                    ...
                                    <tbody data-repeater-list="categories" id="category-tbody">
                                        @php
                                            $categoryCount = $categoryDetails->count();
                                            // Check if categories are empty, if so, show one empty row
                                            if($categoryCount == 0) {
                                                $categoryCount = 1;
                                                $categoryDetails = [new stdClass()]; // Dummy object for loop
                                                $categoryDetails[0]->id = 0;
                                                $categoryDetails[0]->chart_account_id = null;
                                                $categoryDetails[0]->description = '';
                                                $categoryDetails[0]->price = '';
                                                $categoryDetails[0]->billable = 0;
                                                $categoryDetails[0]->tax = 0;
                                                $categoryDetails[0]->customer_id = null;
                                            }
                                        @endphp
                                        @foreach ($categoryDetails as $i => $category)
                                            <tr data-repeater-item>
                                                <td>
                                                    ...
                                                    {{ Form::hidden("category[{$i}][id]", $category->id ?? '') }}
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    <select name="category[{{ $i }}][account_id]"
                                                        ...
                                                        <option value="">{{ __('Select account') }}</option>
                                                        @foreach ($chartAccounts as $id => $account)
                                                            <option value="{{ $id }}" {{ ($category->chart_account_id ?? '') == $id ? 'selected' : '' }}>{{ $account }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                <td>
                                                    {{ Form::textarea("category[{$i}][description]", $category->description ?? '', ['class' => 'form-control', 'rows' => 1]) }}
                                                </td>
                                                <td>
                                                    {{ Form::number("category[{$i}][amount]", $category->price ?? '', ['class' => 'form-control text-end category-amount', 'step' => '0.01']) }}
                                                </td>
                                                <td class="text-center">
                                                    {{ Form::checkbox("category[{$i}][billable]", 1, $category->billable ?? false, ['class' => 'form-check-input']) }}
                                                </td>
                                                <td class="text-center">
                                                    {{ Form::checkbox("category[{$i}][tax]", 1, $category->tax ?? false, ['class' => 'form-check-input']) }}
                                                </td>
                                                <td>
                                                    <select name="category[{{ $i }}][customer_id]"
                                                        ...
                                                        <option value="">-</option>
                                                        @foreach ($customers as $id => $name)
                                                            <option value="{{ $id }}" {{ ($category->customer_id ?? '') == $id ? 'selected' : '' }}>{{ $name }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                    ...
                                    <tbody data-repeater-list="items" id="item-tbody">
                                        ...
                                        @foreach ($items as $i => $item)
                                            <tr data-repeater-item>
                                                <td>
                                                    <span class="text-muted me-2 drag-handle"
                                                        style="cursor: move; font-size: 18px;"><svg
                                                            ...
                                                            <path fill="currentColor"
                                                                d="m10.636 4.565-.07.071a1.93 1.93 0 0 0 0 2.728l.07.07a1.93 1.93 0 0 0 2.728 0l.07-.07a1.93 1.93 0 0 0 0-2.728l-.07-.07a1.93 1.93 0 0 0-2.729 0M4.636 4.565l-.07.071a1.93 1.93 0 0 0 0 2.728l.07.07a1.93 1.93 0 0 0 2.728 0l.071-.07a1.93 1.93 0 0 0 0-2.728l-.07-.07a1.93 1.93 0 0 0-2.729 0M16.636 4.565l-.071.071a1.93 1.93 0 0 0 0 2.728l.07.07a1.93 1.93 0 0 0 2.728 0l.071-.07a1.93 1.93 0 0 0 0-2.728l-.07-.07a1.93 1.93 0 0 0-2.729 0M10.636 10.565l-.071.071a1.93 1.93 0 0 0 0 2.728l.07.07a1.93 1.93 0 0 0 2.729 0l.07-.07a1.93 1.93 0 0 0 0-2.728l-.07-.07a1.93 1.93 0 0 0-2.729 0">
                                                            </path>
                                                            <path fill="currentColor"
                                                                d="m10.636 10.565-.071.071a1.93 1.93 0 0 0 0 2.728l.07.07a1.93 1.93 0 0 0 2.729 0l.07-.07a1.93 1.93 0 0 0 0-2.728l-.07-.07a1.93 1.93 0 0 0-2.729 0M4.636 10.565l-.07.071a1.93 1.93 0 0 0 0 2.728l.07.07a1.93 1.93 0 0 0 2.728 0l.07-.07a1.93 1.93 0 0 0 0-2.728l-.07-.07a1.93 1.93 0 0 0-2.728 0M16.636 10.565l-.07.071a1.93 1.93 0 0 0 0 2.728l.07.07a1.93 1.93 0 0 0 2.728 0l.07-.07a1.93 1.93 0 0 0 0-2.728l-.07-.07a1.93 1.93 0 0 0-2.728 0M10.636 16.565l-.07.071a1.93 1.93 0 0 0 0 2.728l.07.07a1.93 1.93 0 0 0 2.728 0l.07-.07a1.93 1.93 0 0 0 0-2.728l-.07-.07a1.93 1.93 0 0 0-2.728 0">
                                                            </path>
                                                ...
                                                <td>
                                                    {{ Form::number("items[{$i}][price]", $item->price ?? '', [
                                                ...
                                                <td class="text-center">
                                                    {{ Form::checkbox("items[{$i}][tax]", 1, $item->tax ?? false, ['class' => 'form-check-input']) }}
                                                </td>
                                                ...
                                                <td>
                                                    <select name="items[{{ $i }}][customer_id]"
                                                        ...
                                                        <option value="">-</option>
                                                        @foreach ($customers as $id => $name)
                                                            <option value="{{ $id }}" {{ ($item->customer_id ?? '') == $id ? 'selected' : '' }}>{{ $name }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                <td>
                                                    <a href="#" data-repeater-delete
                                                        class="text-danger ti ti-trash"></a>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
    ...
    <div class="modal-footer-custom fixed-footer">
        ...
        <div class="footer-right d-flex align-items-center gap-2">
            ...
            <div class="btn-group">
                <button type="submit"
                    class="btn btn-success btn-sm-qbo">{{ __('Save and close') }}</button>

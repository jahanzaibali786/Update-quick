...
{{-- Vendor selection for Vendor Credit --}}
<script>
   ...
   <div class="modal fade" id="expense-modal" tabindex="-1" aria-labelledby="expenseModalLabel" aria-hidden="true"
    ...
    <div class="d-flex justify-content-between align-items-center border-bottom"
        ...
        <div class="TrowserHeader d-flex align-items-center">
            ...
            <h5 class="mb-0" style="font-size: 1.2rem;">Vendor Credit</h5>
        </div>
        <div class="TrowserHeader d-flex align-items-center">
    ...
    {{ Form::open(['route' => ['vendor-credit.update', $vendorCredit->id], 'method' => 'PUT', 'class' => 'w-100', 'style' => 'padding: 30px 30px; background: #ffffff;', 'id' => 'vendor-credit-form']) }}
    <div class="col-12">
        <input type="hidden" name="_token" id="token" value="{{ csrf_token() }}">
        <input type="hidden" name="vendor_credit_id" value="{{ $vendorCredit->id }}">
        <div class="card">
            <div class="card-body" style="background:#f4f5f8;">

                <!-- ========== TOP SECTION (QBO Vendor Credit Style) ========== -->
                <div class="row align-items-start">

                    <!-- LEFT: VENDOR + MAILING ADDRESS + DATE + REF -->
                    <div class="col-10">
                        <div class="row">
                            {{-- Vendor Dropdown --}}
                            <div class="col-3 form-group mb-3">
                                <label for="vender_id" class="form-label">{{ __('Vendor') }}</label>
                                <select id="vender_id" name="vender_id" class="form-control select" required
                                    data-url="{{ route('bill.vender') }}"
                                    data-create-url="{{ route('vender.create') }}"
                                    data-create-title="{{ __('Add New Vendor') }}">
                                    <option value="">{{ __('Choose a vendor') }}</option>
                                    <option value="__add__">➕ {{ __('Add New Vendor') }}</option>
                                    @foreach ($venders as $id => $name)
                                        <option value="{{ $id }}" {{ ($vendorCredit->vender_id ?? '') == $id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            {{-- Mailing Address --}}
                            <div class="col-3 form-group mb-3">
                                <label for="mailing_address" class="form-label">{{ __('Mailing address') }}</label>
                                <textarea id="mailing_address" name="mailing_address" class="form-control" rows="4" readonly>{{ $vendorAddress ?? '' }}</textarea>
                            </div>

                            {{-- Payment Date --}}
                            <div class="col-2 form-group mb-3">
                                <label for="date" class="form-label">{{ __('Payment date') }}</label>
                                {{ Form::date('date', $vendorCredit->date ?? date('Y-m-d'), ['class' => 'form-control', 'required' => 'required', 'id' => 'date']) }}
                            </div>

                            {{-- Ref No --}}
                            <div class="col-2 form-group mb-3">
                                <label for="ref_no" class="form-label">{{ __('Ref no.') }}</label>
                                <input type="text" name="ref_no" id="ref_no" class="form-control" value="{{ $vendorCredit->vendor_credit_id ?? '' }}">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-2 text-end">
                        <div class="d-flex flex-column align-items-end">
                            <label class="form-label mb-0" style="color:#6b6c72;font-size:12px;">CREDIT AMOUNT</label>
                            <p class="h3 mb-0 grand-total-display" style="font-size:36px;font-weight:900;color:#2ca01c;">${{ number_format($vendorCredit->amount ?? 0, 2) }}</p>
                        </div>
                    </div>
                </div>
        ...
        <div class="row" style="padding:20px;">
            ...
            <div class="col-md-3">
                <div class="form-group">
                    ...
                    {{ Form::textarea('memo', $vendorCredit->memo ?? '', [
                        'class' => 'form-control',
                        'rows' => '5',
                        'id' => 'bill_memo',
                        'maxlength' => '4000',

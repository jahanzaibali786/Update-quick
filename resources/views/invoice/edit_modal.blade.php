        ...
        <script>
            $(document).ready(function() {
                ...
                @if(isset($invoiceData) && $invoiceData)
                    ...
                    // console.log('Total items to load:', invoiceData.items ? invoiceData.items.length : 0);

                    // Populate header fields immediately
                    if (invoiceData.customer_id) {
                        $('#customer_id').val(invoiceData.customer_id).trigger('change');
                        $('#customer_id').trigger('change');
                    }

                    if (invoiceData.issue_date) {
                        $('input[name="issue_date"]').val(invoiceData.issue_date);
                    }

                    if (invoiceData.due_date) {
                        $('input[name="due_date"]').val(invoiceData.due_date);
                    }

                    if (invoiceData.ref_number) {
                        $('input[name="ref_number"]').val(invoiceData.ref_number);
                    }

                    if (invoiceData.category_id) {
                        $('input[name="category_id"]').val(invoiceData.category_id);
                    }

                    if (invoiceData.terms) {
                        $('select[name="terms"]').val(invoiceData.terms);
                    }

                    if (invoiceData.memo) {
                        $('textarea[name="memo"]').val(invoiceData.memo);
                    }

                    if (invoiceData.note) {
                        $('textarea[name="note"]').val(invoiceData.note);
                    }
                    ...
                    if (invoiceData.logo) {
                        ...
                        $logoPreview.attr('src', invoiceData.logo).removeClass('d-none');
                        $addText.addClass('d-none');
                        $sizeText.addClass('d-none');
                        $removeBtn.removeClass('d-none');
                        $logoButton.addClass('has-logo');
                    ...
                    if (invoiceData.attachments && invoiceData.attachments.length > 0) {
                        ...
                        invoiceData.attachments.forEach(function(attachment) {
                            var rowId = 'existing_att_' + attachment.id;
                            ...
                            var $row = $(
                                ...
                                'type="checkbox" ' +
                                'name="existing_attachments_email[' + attachment.id + ']" ' +
                                (attachment.attach_to_email ? 'checked' : '') + '>' +
                        ...
                        // Show header if attachments exist
                        if (invoiceData.attachments.length > 0) {
                            $header.removeClass('d-none');
                        }

                        // Handle removal of existing attachments
                        $(document).on('click', '.attachment-remove-existing', function() {

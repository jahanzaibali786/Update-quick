...
<div class="row">
    ...
    {{ Form::open(['url' => 'bill', 'class' => 'w-100', 'style' => 'padding: 30px 30px; background: #ffffff;', 'id' => 'bill-form']) }}
    <div class="col-12">
        ...
        <div class="card">
            <div class="card-body" style="background:#f4f5f8;">
                ...
                <div class="bill-header-section">
                    {{-- Top Row: Vendor + Balance Due --}}
                    <div class="bill-header-top">
                        <div class="bill-vendor-col">
                            <div class="bill-field-group">
                                <label>{{ __('Vendor') }}</label>
                                <select name="vender_id" id="vender" class="form-control select2" required data-url="{{ route('bill.vender') }}">
                        ...
                        <div class="bill-balance-col">
                            <div class="bill-balance-label">{{ __('BALANCE DUE') }}</div>
                            <div class="bill-balance-amount grand-total-display">{{ \Auth::user()->priceFormat($bill->total) }}</div>
                        </div>
                    </div>

                    {{-- Collapsible Section: Address, Terms, Dates, Bill No --}}
                    <div class="bill-collapsible" id="billDetailsCollapsible">
                        <div class="bill-details-section">
                            <div class="bill-field-group bill-address-field">
                                <label>{{ __('Mailing address') }}</label>
                                <textarea name="mailing_address" id="mailing_address" class="form-control" placeholder="">{{ $vendorOptions[$bill->vender_id]['address'] ?? '' }}</textarea>
                            </div>
                            <div class="bill-field-group bill-terms-field">
                                <label>{{ __('Terms') }}</label>
                                <select name="terms" id="terms" class="form-control">
                                    <option value=""></option>
                                    @foreach($paymentTerms as $id => $name)
                                        <option value="{{ $id }}" {{ $bill->terms == $id ? 'selected' : '' }}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="bill-field-group bill-date-field">
                                <label>{{ __('Bill date') }}</label>
                                <input type="date" name="bill_date" id="bill_date" class="form-control" required value="{{ $bill->bill_date }}">
                            </div>
                            <div class="bill-field-group bill-date-field">
                                <label>{{ __('Due date') }}</label>
                                <input type="date" name="due_date" id="due_date" class="form-control" required value="{{ $bill->due_date }}">
                            </div>
                            <div class="bill-field-group bill-billno-field">
                                <label>{{ __('Bill no.') }}</label>
                                <input type="text" name="order_number" class="form-control" value="{{ $bill->order_number }}">
                            </div>
                        </div>
                    </div>
...
<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" color="currentColor" width="20px" height="20px" focusable="false" aria-hidden="true" class=""><path fill="currentColor" d="M18.014 15.991a1 1 0 0 1-.708-.294l-5.285-5.3-5.3 5.285a.999.999 0 1 1-1.412-1.416l6.008-5.99a1 1 0 0 1 1.414 0l5.992 6.008a1 1 0 0 1-.708 1.706z"></path></svg>
                ...
                <script>
                    ...
                    $(document).ready(function() {
                        ...
                        var termsData = @json($termsData);
                        
                        // Handle Vendor Change
                        $('#vender').on('change', function() {
                            var vendorId = $(this).val();
                            if (vendorId && vendorData[vendorId]) {
                                var data = vendorData[vendorId];
                                
                                // Update Address
                                $('#mailing_address').val(data.address);
                                
                                // Handle Terms auto-selection
                                if (data.terms) {
                                     $("#terms option").filter(function() {
                                         return $(this).text().trim() == data.terms; 

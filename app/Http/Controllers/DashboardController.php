        ...
        if (Auth::check()) {
            if (Auth::user()->type == 'super admin') {
                ...
                if (\Auth::user()->can('show account dashboard')) {
                    ...
                    // dd($exColor,$exAmount,$exCategory);

                    $data['expenseCategoryColor'] = $exColor;
                    $data['expenseCategory'] = $exCategory;
                    $data['expenseCatAmount'] = $exAmount;
                    $data['incweekBarChartData'] = \Auth::user()->getincweekBarChartData();
                    $data['incmonthBarChartData'] = \Auth::user()->getincmonthBarChartData();
                    $data['incyearBarChartData'] = \Auth::user()->getincyearBarChartData();
                    ...
                    $data['yearlyInvoice']    = \Auth::user()->yearlyInvoice();

                    // dd('asd',$data['monthlyInvoice']); 
                    // dd('weekly',$data['weeklyInvoice'], 'monthly',$data['monthlyInvoice'],'yearly',$data['yearlyInvoice']);
...
//                $user_deal   = $deals->pluck('deal_id')->toArray();
                ...
                return view('dashboard.crm-dashboard', compact('crm_data','targets'));
            }
        } else {
            return $this->account_dashboard_index();
        }

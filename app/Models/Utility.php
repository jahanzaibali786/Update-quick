...
//    }
    ...
    public static function makeNotification($user_id, $type, $data, $data_id, $action_type)
    {
        ...
        Notification::create([
            ...
            'data' => json_encode($data),  // nnotifiation array
            'data_id' => $data_id, // module id
            'notification_for' => $action_type, // type of notification
            'is_read' => 0, // seen unseen
        ]);
    }
    ...
    public static function brv_entry($data)
    {
        ...
        //     $journalItem              = new JournalItem();
        //     $journalItem->journal     = $journal->id;
        //     $journalItem->account     = $tax_wth->account_id;
        //     $journalItem->description = 'WTH Tax on Invoice No : '.@$data['no'];
        //     $journalItem->prod_tax_id = $data['prod_id'];
        //     $journalItem->credit       = 0;
        //     $journalItem->debit        = $data['wth'];
        //     $journalItem->save();
        // }
        ...
        //     $products = InvoiceProduct::where('invoice_id',$data['id'])->get();
        //     foreach ($products as $product) {
        //         $tax = Tax::find($product->tax);
        //         if ($tax) {
        //             $taxPrice  = (@$tax->rate / 100) * (@$product->price * @$product->quantity);
        //             $journalItem              = new JournalItem();
        //             $journalItem->journal     = $journal->id;
        //             $journalItem->account     = @$tax->account_id;
        //             $journalItem->prod_tax_id  = @$product->id;
        //             $journalItem->description  = 'Product Tax on Invoice No : ' . @$data['no'];
        //             $journalItem->credit       = @$taxPrice;
        //             $journalItem->debit        =  0;
        //             $journalItem->save();

        //             $item_last = JournalItem::where('product_ids', $product->id)->first();
        //             $item_last->credit = ($item_last->credit - @$taxPrice);
        //             $item_last->save();
        //         }
        //     }
        // }
    ...
    public static function bpv_entry($data)
    {
        ...
        $journal->date = $data['date'];
        $journal->reference = $data['reference'];
        $journal->description = 'Payment on ' . $data['category'] . ' No : ' . @$data['no'];
        $journal->reference_id = $data['id'];
        $journal->category = $data['category'];
    ...
    public static function jr_exp_entry($data)
    {
        ...
        $journal->date = $data['date'];
        $journal->reference = $data['reference'];
        $journal->description = 'Bill No : ' . @$data['no'];
        $journal->reference_id = $data['id'];
        $journal->category = $data['category'];
        $journal->voucher_type = 'JV';
        // $journal->prod_id       =  $data['prod_id'];
